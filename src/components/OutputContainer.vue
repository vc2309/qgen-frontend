<template>
  <div ref="scrollContainer"
    class="p-2 mx-auto my-auto h-5/6 w-11/12 rounded-md from-slate-950 text-left overflow-y-auto ">
    <div v-show="(currConversationLength() == 0) && !loading" class=" mt-20 text-center text-white text-xl">Submit your
      article to generate questions!</div>
    <div v-show="currConversationLength() > 0" v-for="item in getCurrentConversationQuestions()" class="p-2">
      <div class="p-4 w-full h-fit border-white border-solid border rounded-xl hover:bg-white hover:text-black ">{{ item
      }}</div>
    </div>
  ...
  <div class="mt-10 font-extrabold text-xl">{{ currConversation + 1 }}</div>
</template>
<script>
